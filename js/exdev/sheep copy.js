...
class Sheep{
    ...
    ruleMerge(seperation, cohesion, alignment, escape, predDistance){
        ...
        finalVel = this.add(finalVel, this.multiply(escape, 50));
    ...
    checkRotation(){
        ...
        var nangle = Math.acos((this.sprite.x* this.newVel.x + this.sprite.y * this.newVel.y)/(this.magnitude(this.sprite) * (this.magnitude(this.newVel))))
        console.log(this.newVel)
        var clockwise = angle <= 10;
        var anticlockwise = angle >= - 10;
        if (clockwise && anticlockwise){
            return this.newVel;
        }else if(!clockwise){
            return {x:Math.cos(currAngle + 0.174533), y:Math.sin(currAngle +0.174533)};
        }else{
            return {x:Math.cos(currAngle - 0.174533), y:Math.sin(currAngle - 0.174533)};
        }
    }
